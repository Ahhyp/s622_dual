    ...
    double search_discretization)
{
    ...
    // 2026-08-25 双臂支持：grasp_frame 可能带 left_/right_ 前缀，逐个尝试
    // （check 仅为诊断，不影响 IK 求解；单臂场景仍是 "grasp_frame"）
    const std::vector<std::string> grasp_candidates = {
        "grasp_frame", "left_grasp_frame", "right_grasp_frame"};
    bool checked_any = false;
    for (const auto& gf_name : grasp_candidates) {
        const auto* grasp_link = robot_model.getLinkModel(gf_name);
        ...
        if (pos_err > 1.0e-4 || rot_err > 1.0e-4) {
            RCLCPP_WARN(
                node->get_logger(),
                "Fairino IK tool/URDF mismatch: configured wrist3_link->%s %s, urdf %s, pos_err=%.6f, rot_err=%.6f",
                gf_name.c_str(),
            ...
            RCLCPP_INFO(
                node->get_logger(),
                "Fairino IK tool/URDF check ok: wrist3_link->%s pos_err=%.6f, rot_err=%.6f",
                gf_name.c_str(),
                pos_err,
                rot_err);
        }
    ...
    if (!checked_any) {
        RCLCPP_WARN(
            node->get_logger(),
            "Fairino IK tool/URDF check skipped: no grasp_frame (or left_/right_grasp_frame) "
            "fixed under wrist3_link");
    }
...
ToolModel FairinoIKPlugin::resolveToolModel(const std::string& tip_frame) const {
    ...
    // 自动判断：根据常见的末端连杆名称
    // 夹爪相关的名称
    // 2026-08-25 双臂支持：left_grasp_frame / right_grasp_frame 以 _grasp_frame 结尾
