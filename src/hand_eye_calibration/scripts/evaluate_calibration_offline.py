...
Modes:
  * re-solve hold-out (default): split the dataset into solve / hold-out
    subsets, solve the hand-eye on the solve subset only, score the hold-out
    subset against the solve-set reference frame.  This is the honest
    independent metric — samples used for solving never score themselves.
  * saved-calibration check (``--calib``): report constant-frame consistency
    of the samples against an existing ``.calib`` hand-eye.  Only meaningful
    when the samples were NOT used to compute that calibration (e.g. a fresh
    hold-out run on the real robot).
...
def _main(argv=None) -> int:
    ...
    if args.solve_count is None:
        if args.calib:
            ...
            handeye = evaluator.load_calibration_yaml(args.calib)
            metrics = evaluator.constant_frame_metrics(samples, handeye)
            print("=" * 62)
            print(f"  Saved-calibration check: {args.calib}")
    ...
    if args.calib:
        ...
        delta_t = saved.translation - result.handeye.translation
        print(f"  Re-solved vs saved {args.calib}:")
        print(f"    translation delta = {rotation_delta_deg(saved.rotation, result.handeye.rotation):.3f} deg "
