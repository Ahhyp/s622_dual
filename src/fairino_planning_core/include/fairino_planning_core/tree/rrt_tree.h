...
public:
    ...
    // 添加节点, 返回索引
    int addNode(const JointArray& state, int parent, double cost);
    ...
    // 范围搜索
    std::vector<int> nearRadius(const JointArray& q, double radius) const;
    ...
    // 路径回溯
    std::vector<JointArray> backtrack(int leaf_idx) const;
