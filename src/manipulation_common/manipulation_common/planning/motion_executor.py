...
class MoveItMotion:
    ...
    def move_to_pose(
        ...
        try:
            ...
            if plan_only:
                self._log_plan_only(best_path, action_name)
                return True
    ...
    def _log_plan_only(self, best_path, action_name: str) -> None:
        """打印规划结果（不执行）。用于 Plan Only 安全模式。"""
        num_points = len(getattr(best_path, "points", []) or [])
        duration = 0.0
        if num_points > 0:
            t = getattr(best_path.points[-1], "time_from_start", None)
            if t is not None:
                duration = float(getattr(t, "sec", 0)) + float(
                    getattr(t, "nanosec", 0)
                ) * 1e-9
        self.node.get_logger().info(
            f"{action_name}: [PLAN ONLY] 规划成功：{num_points} 个路径点，"
            f"时长 {duration:.3f}s — 已跳过执行"
        )
