...
#### Phase 1b 扫频结果（2026-08-28 执行，真机 58.3，保持位，各 2000 次）

| 频率 | stall(>20ms) | 每 stall 平均间隔 | stall cycle 规律 | queue |
|---|---|---|---|---|
| 50Hz | 14 | ~2.0s | 后半段 380→1381 连续 **+100** | 恒 0 |
| 100Hz | 10 | ~1.4-3.4s（散） | 散（253→592→827…） | 恒 0（2 次 2/6） |
| 125Hz | 17 | ~1.3s | 1050→1950 连续 10 次 **+100** 完美 | 恒 0 |
| 250Hz | 17 | ~1.3s（个别 2.5s） | ~+100~200，多数接近 +100 | 恒 0 |

**判定**：

1. **所有频率都 stall（10-17 次/2000）→ 与发送频率无关**，低频不消除
2. **queue 几乎恒为 0 → 排除"队列满"假设**（用户判读表第 5 行命中）
3. **stall cycle 在 125Hz 完美 +100、50Hz 后半段 +100 → 强支持"每 ~100 次 ServoJ 调用触发一次 ~1s stall"（调用计数机制）**；偶发跳过一次（+200）
